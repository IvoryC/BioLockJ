...
calculateStats <- function( level ) {
	...
	if ( is.null(taxaTable) || is.null(metaTable) ){
		return(NULL)
	}
	...
	logInfo( "number of taxa columns", ncol(taxaTable) )
	...
	for( taxaCol in 1:ncol(taxaTable) ) {
		if( sum( taxaTable[,taxaCol] > 0 ) >= cutoffValue ) {
			...
			if( length( binaryCols ) > 0 ) {
				...
				for( attName in binaryCols ) {
					att = as.factor( metaTable[,attName] )
					vals = levels( att )
			...
			if( length( numericCols ) > 0 ) {
				...
				for( attName in numericCols ) {
					att = as.numeric( metaTable[,attName] )
