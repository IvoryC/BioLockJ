...
addPageFooter <- function (main, level, pageNum){
	pageNumberText = paste(main, level, paste("page:", pageNum), sep=" - ")
	mtext(pageNumberText, side=1, outer=TRUE, line=0, adj = 1)
}

# Add a page title
addPageTitle <- function (main, level=NULL){
	mtext(main, side=3, outer = TRUE, font=par("font.main"), cex=par("cex.main"), line=2)
	if (!is.null(level)){
			titlePart2 = paste( "taxonomic level:", level)
