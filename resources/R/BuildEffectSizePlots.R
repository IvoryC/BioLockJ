...
main <- function(){
  ...
  useAdjustedPs = !getProperty("r.plotEffectSize.disablePvalAdjustment", FALSE)
  ...
  doFoldChange = !getProperty("r.plotEffectSize.disableFoldChange", FALSE) 
  doCohensD = !getProperty("r.plotEffectSize.disableCohensD", FALSE)
  doRSquared = !getProperty("r.plotEffectSize.disableRSquared", FALSE)
  ...
  for (level in taxaLevels() ) {
    ...
    logInfo( c("taxaTable has", nrow(meta), "rows and", ncol(meta), "columns.") )
